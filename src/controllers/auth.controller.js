...
export const updateStoreAdminCredentials = async (req, res) => {
    try {
        const userId = req.user?.id;
        const { email, username, currentPassword, newPassword } = req.body;
        ...
        // Verify Store Admin role
        if (user.role && user.role.toUpperCase() !== "STORE_ADMIN" && user.role.toUpperCase() !== "STOREADMIN") {
            return res.status(403).json({
                success: false,
        ...
        console.error("Update Store Admin Credentials Error:", error);
